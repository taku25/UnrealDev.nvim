...
builder.create({
  ...
  subcommands = {
    ...
    ["check_includes"] = {
      handler = function()
        api.check_includes(vim.api.nvim_get_current_buf())
      end,
      desc = "UCM: Check for missing #include directives in the current buffer.",
      args = {},
    },
    ["fix_includes"] = {
      handler = function()
        api.fix_includes(vim.api.nvim_get_current_buf())
      end,
